...
function usage {
    echo -e "usage : hicpro2juicebox -i VALIDPAIRS -g GSIZE -j JUICERJAR -l RESOLUTION [-r RESFRAG] [-t TEMP] [-o OUT] [-h]"
...
function help {
    ...
    echo "   -l|--res RESOLUTION : resolution. Default is juicer pre default resolution"
...
while getopts ":i:r:g:j:t:o:l:ch" OPT
do
    case $OPT in
	i) VALIDPAIRS=$OPTARG;;
	g) GSIZE=$OPTARG;;
...
date=$( date  +%Y%m%d  )
OUTPUTFILE=${OUT}/$(basename $VALIDPAIRS).$date.hic
...
if [[ ! -z $RESFRAG ]]; then
  ...
  if [[ ! -z $RESOLUTION ]];then
    java -Xmx32g -jar ${JUICERJAR} pre -r $RESOLUTION -f ${TEMP}/$$_resfrag.juicebox ${TEMP}/$$_allValidPairs.pre_juicebox_sorted ${OUTPUTFILE} ${GSIZE}
  else
  ...
  if [[ ! -z $RESOLUTION ]];then
    java -Xmx32g -jar ${JUICERJAR} pre -r $RESOLUTION  ${TEMP}/$$_allValidPairs.pre_juicebox_sorted ${OUTPUTFILE} ${GSIZE}
  else
