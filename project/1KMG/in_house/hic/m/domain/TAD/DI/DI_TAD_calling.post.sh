...
chrLst=$( cut -f1 iced/bin${binSize}_win${winSize}/HMM/merged_${name}.7col | sort | uniq  | tr "\n" " " )
...
do
	...
	sed -i 's/chr21/chrM/g;s/chr22/chrX/g;s/chr23/chrY/g'  ./$normal/bin${binSize}_win${winSize}/domain/merged_${name}.domain
done
else
